...
(function ($, rulesEngine) {
    ...
    validation.wordLength = function (options, word, score) {
        var wordlen = word.length,
            lenScore = 0;//Math.pow(wordlen, options.rules.raisePower);
    ...
    validation.wordLengthMinEight = function (options, word, score) {
        var wordlen = word.length,
            lenScore = 0;//Math.pow(wordlen, options.rules.raisePower);
        if (wordlen >= options.common.minChar) {
            lenScore = (lenScore + score);
    ...
    validation.wordLengthMoreThanEight = function (options, word, score) {
        var wordlen = word.length,
            lenScore = 0;//Math.pow(wordlen, options.rules.raisePower);
        if (wordlen > options.common.minChar) {
            lenScore = (lenScore + score);
...
defaultOptions.rules.scores = {
    wordNotEmail: 0,
    wordLength: 0,
    wordLengthMinEight: 20,
    wordLengthMoreThanEight: 2,
...
defaultOptions.rules.activated = {
    wordNotEmail: false,
    wordLength: false,
    wordLengthMinEight: true,
    wordLengthMoreThanEight: true,
...
defaultOptions.ui.verdicts = ["Weak", "Medium", "Medium", "Strong", "Strong"];
...
defaultOptions.ui.scores = [35, 38, 43, 44];
...
(function ($, ui) {
	...
	if (window.innerWidth <= 1024) {			
				//console.log("mobile");
				var pwStrengthTooltipPlacement = 'bottom';
	...
	if (window.innerWidth > 1024) {			        
				//console.log("desktop");
				var pwStrengthTooltipPlacement = 'right';
    ...
    ui.initPopover = function (options, $el) {
        ...
        $el.popover({
			...
			placement: pwStrengthTooltipPlacement,
            trigger: "manual",
            content: " "
        });
    };
